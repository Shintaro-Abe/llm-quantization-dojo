...
3. **第1章 座学**: NF4/QLoRA の概念解説（index.md）と公式リンク集（references.md・最終確認日付き）。NF4とQLoRAの混同防止、トラブルシュート表、**モデル選定の安全チェック（safetensors/非ゲート/公式org/`trust_remote_code`不要）**のコラムを含む。
4. **第1章 ハンズオン**: Colabで完走する Notebook（4bitロード→QLoRA微調整→推論の前後比較）。
5. **進捗管理テンプレ＋手順書**: 章タスク/内容修正のIssueテンプレート、Projects v2 運用手順（**Board=進捗一覧＋Roadmap=ガントチャート**）。※PRテンプレートは1人運用の摩擦のため**作らない**（品質ゲートはCIが担保）。
...
- AC-3: 既定モデルは無料T4で確実に完走するサイズ（1〜3B級・ログイン不要な非ゲート/Apache・MIT）。**既定は西側 Apache モデル `HuggingFaceTB/SmolLM2-1.7B`**（フォールバック `TinyLlama-1.1B-Chat`、発展 `Qwen2.5-1.5B`）。7Bは発展課題として案内する。
...
- 使用モデルは **safetensors・非ゲート・Apache-2.0/MIT・`trust_remote_code` 不要**を既定とする（grilling=方針X／座学の安全チェックに準拠）。
