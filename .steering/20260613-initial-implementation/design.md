...
## 6. ハンズオンNotebookテンプレート設計（grilling 反映 / 実装済み）

実装ファイル: `notebooks/01_bitsandbytes_qlora.ipynb`（全16セル）。

| セル | 内容 |
| :-- | :-- |
| 先頭(md) | タイトル＋Colabバッジ＋メタ（所要時間/必要VRAM/対象モデル/最終確認日）＋**検証状況の明示** |
| 概要(md) | やること／デモのねらい／**過学習を使う極端なデモである旨の正直注記** |
| セットアップ | `!pip install`（量子化/学習系を**緩めに下限指定** `>=`、torchはColab既定に従う） |
| 環境確認 | `!nvidia-smi`・`seed` 固定・GPU assert |
| 本編1 | NF4で4bitロード（`BitsAndBytesConfig`、`compute_dtype=fp16`＝T4対応） |
| 本編2 | 微調整「前」の応答（未知の質問で生成） |
| 本編3 | 小データ**十数件を直書き**→ `Dataset` 化・トークナイズ |
| 本編4 | QLoRA（`prepare_model_for_kbit_training`→`LoraConfig`→`Trainer`、`paged_adamw_8bit`） |
| 本編5 | 微調整「後」の応答（同じ質問で前後比較） |
| 末尾(md) | 理解度確認の設問 → chapter-task Issue作成への導線リンク |

### 確定したデモ設計（grilling）
- **既定モデル**: base `HuggingFaceTB/SmolLM2-1.7B`（baseの方が変化が鮮明）。OOM時 `TinyLlama-1.1B-Chat`。
- **デモタスク**: 「どんな質問にも語尾を必ず『〜だミャ。』で1文」に強制 → 前後差が一目瞭然。
- **dtype**: T4(Turing)はbf16非対応のため `bnb_4bit_compute_dtype=torch.float16` / `TrainingArguments(fp16=True)`。
- **データ**: 外部依存ゼロのため**直書き十数件**（即再現・著作権安全）。
- **正直注記**: 過学習を利用した可視化デモであり、実務は多様な十分量データ＋評価が必要、と明記。
- **検証分担**: 作者=**静的検証（nbformat妥当＋各pythonセルの構文）＋ロジック検証**まで。**T4実機の Run all 完走は学習者が確認**（Notebook冒頭に明示）。
- Colabバッジ URL: `https://colab.research.google.com/github/Shintaro-Abe/llm-quantization-dojo/blob/main/notebooks/01_bitsandbytes_qlora.ipynb`（実slug差し込み済み。座学 index.md にも同バッジ）。
